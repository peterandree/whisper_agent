...
def _log_device(label: str, device: str) -> None:
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            logger.warning(
                f"{label} — nvidia-smi returned non-zero. stderr: {result.stderr.strip()}"
            )
...
def transcribe(path: Path, device: str, compute_type: str, hf_token: str) -> str:
    ...
    try:
        ...
        logger.error(
            f"Audio preprocessing with ffmpeg failed: {e}\n"
            "Check that ffmpeg is installed and the input file is a valid audio file. "
            "Using original file."
        )
        audio_path = path
    ...
    try:
        ...
        if "CUDA out of memory" in str(e):
            logger.error("CUDA OOM during transcription. Try a smaller model or CPU.")
        raise
    except Exception as e:
        if "CUDA out of memory" in str(e):
            logger.error("CUDA OOM during transcription. Try a smaller model or CPU.")
    ...
    if skip_alignment:
        ...
        try:
            ...
            if "CUDA out of memory" in str(e):
                logger.error("CUDA OOM during alignment. Try a smaller model or CPU.")
    ...
    try:
        ...
        if "CUDA out of memory" in str(e):
            logger.error("CUDA OOM during diarization. Try a smaller model or CPU.")
